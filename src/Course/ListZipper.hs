...
findLeft p (ListZipper l x r) =
  case break p l of
    (_, Nil)         -> IsNotZ
    (rs, (nx :. xs)) -> IsZ (ListZipper xs nx (reverse rs ++ pure x ++ r))
...
findRight p (ListZipper l x r) =
  case break p r of
    (_, Nil)         -> IsNotZ
    (ls, (nx :. xs)) -> IsZ (ListZipper (reverse ls ++ pure x ++ l) nx xs)

reverseNonEmpty ::
  a
  -> List a
  -> (a, List a)
reverseNonEmpty x Nil =
  (x, Nil)
reverseNonEmpty x l =
  case reverse l of
    Nil        -> (x, Nil)
...
moveLeftN ::
  ...
  -> MaybeListZipper a
moveLeftN n l
  | n > 0     = next (moveLeft l)  (n - 1)
  | n < 0     = next (moveRight l) (n + 1)
  | otherwise = IsZ l
  where
    next IsNotZ = const IsNotZ
    next (IsZ lz) = flip moveLeftN lz
...
moveLeftN' n l =
  iterate l n maxValue
  where
    iterate l' n' m
      | n' > 0    = next (moveLeft l')  (n' - 1) m
      | n' < 0    = next (moveRight l') (n' + 1) m
      | otherwise = Right l'

    next IsNotZ = const Left
    next (IsZ lz) = iterate lz

    maxValue
      | n > 0     = (length . lefts)  l
      | otherwise = (length . rights) l
...
nth ::
  ...
  -> MaybeListZipper a
nth n l =
  let leftLength = length (lefts l)
  in moveLeftN (leftLength - n) l
...
toListNonEmpty ::
  ListZipper a
  -> (a, List a)
toListNonEmpty (ListZipper l x r) =
  case reverse l of
    Nil        -> (x, r)
    (x' :. xs) -> (x', xs ++ pure x ++ r)
...
-- prop> \lz -> rights (end lz) == Nil
  ...
  let (h, l') = toListNonEmpty l
      (x, xs) = reverseNonEmpty h l'
...
insertPushLeft ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushLeft nx (ListZipper l x r) =
  ListZipper (x :. l) nx r
...
insertPushRight ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushRight nx (ListZipper l x r) =
  ListZipper l nx (x :. r)
...
instance Extend ListZipper where
  (<<=) ::
    ...
    -> ListZipper b
  (<<=) f z =
    ListZipper leftExtended (f z) rightExtended
    where
      -- :: (a -> Optional (b, a))
      leftExtended  = unfoldr (tryMove moveLeft)  z
      rightExtended = unfoldr (tryMove moveRight) z
