...
jsonString ::
  Parser Chars
jsonString =
  betweenQuotes (list jsonStringChar)
  where
    betweenQuotes =
      between (is '"') (charTok '"')

    jsonStringChar =
      processJsonChar =<< character

    processJsonChar c
      | c == '"'  = unexpectedCharParser c
      | c == '\\' = hexu ||| specialChar
      | otherwise = pure c

    specialChar =
      processSpecialChar =<< character
...
jsonObject ::
  Parser Assoc
jsonObject =
  betweenSepbyComma '{' '}' kvParser
  where
    kvParser :: Parser (Chars, JsonValue)
    kvParser =
      (,)
        <$> tok jsonString
        <*  charTok ':'
