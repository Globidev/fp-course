...
filtering ::
  ...
  -> f (List a)
filtering p l =
  listOptional keepValid <$> (sequence . (map runPredicate)) l
  where
    -- Runs the predicate and lifts the result with the original value
    runPredicate x = (,) x <$> p x

    keepValid (x, True)  = Full x
    keepValid (_, False) = Empty
